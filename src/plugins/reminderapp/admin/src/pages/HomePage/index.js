...
function HomePage() {
  ...
  async function updateReminder(id, data) {
    await ReminderApiHandler.editReminder(id, {
  ...
  function addReminder(data) {
    ReminderApiHandler.addReminder({
      remindername: data.remindername,
      date: data.date,
      datetime: new Date(data.date),
      isdatepassed: false,
  ...
  return (
    <Box>
      <Layout>
        <Box background="neutral100">
          <BaseHeaderLayout
            ...
            title="Add reminder"
            subtitle={`${reminderList.length} reminders found`}
            as="h2"
          />
        </Box>
      ...
      {ModalVisible && (
        <Modal setShowModal={SetModalVisible} addReminder={addReminder} />
      )}
      {isEdit && (
        <EditModal
          ...
          id={editedVal.id}
          dateGiven={editedVal.date}
          nameGiven={editedVal.remindername}
        />
      )}
    </Box>
  );
